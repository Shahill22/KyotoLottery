...
const KyotoSwapLottery = artifacts.require("./KyotoSwapLottery.sol");
...
contract(
  ...
  ([alice, bob, carol, david, erin, operator, treasury, injector]) => {
    ...
    before(async () => {
      ...
      rewardToken = await MockERC20.new(
        "Mock Reward ",
        "Reward",
        _totalInitSupply
      );
      // Deploy Mock Kyoto
      mockKyoto = await MockERC20.new("Mock Kyoto ", "Kyoto", _totalInitSupply);
      ...
      // Deploy KyotoSwapLottery
      lottery = await KyotoSwapLottery.new(
        mockKyoto.address,
    ...
    describe("LOTTERY #1 - CUSTOM RANDOMNESS", async () => {
      it("Admin sets up treasury/operator address", async () => {
        result = await lottery.setOperatorAndTreasuryAndInjectorAddresses(
          operator,
          treasury,
          injector,
          { from: alice }
      ...
      it("Operator starts lottery", async () => {
        ...
        console.info(
          `        --> Cost to start the lottery: ${gasToBNB(
            result.receipt.gasUsed
          )} (USD: ${gasToUSD(result.receipt.gasUsed)})`
        );
      });
      ...
      it("Bob buys 100 tickets", async () => {
        ...
        expectEvent(result, "TicketsPurchase", {
          buyer: bob,
          lotteryId: "1",
          numberTickets: "100",
        });
        ...
        console.info(
          `        --> Cost to buy the first 100 tickets: ${gasToBNB(
            result.receipt.gasUsed
          )} (USD: ${gasToUSD(result.receipt.gasUsed)})`
        );
      ...
      it("Carol buys 1 ticket", async () => {
        ...
        console.info(
          `        --> Cost to buy a stand-alone ticket: ${gasToBNB(
            result.receipt.gasUsed
          )} (USD: ${gasToUSD(result.receipt.gasUsed)})`
        );
      ...
      it("David buys 10 tickets", async () => {
        ...
        console.info(
          `        --> Cost to buy 10 tickets: ${gasToBNB(
            result.receipt.gasUsed
          )} (USD: ${gasToUSD(result.receipt.gasUsed)})`
        );
      ...
      it("Owner does 10k Kyoto injection", async () => {
        ...
        expectEvent(result, "LotteryInjection", {
          lotteryId: "1",
          injectedAmount: parseEther("10000").toString(),
        });

        console.info(
          `        --> Cost to do injection: ${gasToBNB(
            result.receipt.gasUsed
          )} (USD: ${gasToUSD(result.receipt.gasUsed)})`
        );
      ...
      it("Operator closes lottery", async () => {
        ...
        expectEvent(result, "LotteryClose", {
          lotteryId: "1",
          firstTicketIdNextLottery: "111",
        });

        console.info(
          `        --> Cost to close lottery: ${gasToBNB(
            result.receipt.gasUsed
          )} (USD: ${gasToUSD(result.receipt.gasUsed)})`
        );
      });
      ...
      it("Numbers are drawn (9/9/9/9/9/9)", async () => {
        // 11 winning tickets
        result = await lottery.drawFinalNumberAndMakeLotteryClaimable(
          "1",
          true,
          { from: operator }
        );
        ...
        console.info(
          `        --> Cost to draw numbers (w/o ChainLink): ${gasToBNB(
            result.receipt.gasUsed
          )} (USD: ${gasToUSD(result.receipt.gasUsed)})`
        );
      });
      ...
      it("David claims the jackpot", async () => {
        // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 Reward collected
        // 10053.0025 * (1-0.20) * 0.5 = 4021.201 RewardTokens

        result = await lottery.claimTickets("1", ["110"], ["5"], {
          from: david,
        });
      ...
      it("Bob claims 10 winning tickets he bought", async () => {
        ...
        console.info(
          `        --> Cost to claim 9 tickets: ${gasToBNB(
            result.receipt.gasUsed
          )} (USD: ${gasToUSD(result.receipt.gasUsed)})`
        );
      ...
      describe("LOTTERY #2 - CUSTOM RANDOMNESS - Exceptions", async () => {
        it("Operator cannot close lottery that is in claiming", async () => {
          await expectRevert(
            lottery.closeLottery("1", { from: operator }),
            "Lottery not open"
          );
        });
        ...
        it("Operator cannot inject funds in a lottery that is Open status", async () => {
          await expectRevert(
            lottery.injectFunds("1", parseEther("10"), { from: alice }),
            "Lottery not open"
          );
          await expectRevert(
            lottery.injectFunds("2", parseEther("10"), { from: alice }),
            "Lottery not open"
          );
        });
        ...
        it("User cannot buy 1 ticket for old lottery", async () => {
          await expectRevert(
            lottery.buyTickets("1", ["1999999"], { from: bob }),
            "Lottery is not open"
          );
        });
        ...
        it("User cannot buy 1 ticket for future lottery", async () => {
          await expectRevert(
            lottery.buyTickets("2", ["1999999"], { from: bob }),
            "Lottery is not open"
          );
        });
        ...
        it("User cannot claim a ticket with wrong bracket", async () => {
          await expectRevert(
            lottery.claimTickets("1", ["104"], ["6"], { from: david }),
            "Bracket out of range"
          );
          await expectRevert(
            lottery.claimTickets("1", ["104"], ["5"], { from: david }),
            "No prize for this bracket"
          );
          await expectRevert(
            lottery.claimTickets("1", ["104"], ["4"], { from: david }),
            "No prize for this bracket"
          );
          await expectRevert(
            lottery.claimTickets("1", ["104"], ["3"], { from: david }),
            "No prize for this bracket"
          );
          await expectRevert(
            lottery.claimTickets("1", ["104"], ["2"], { from: david }),
            "No prize for this bracket"
          );
          await expectRevert(
            lottery.claimTickets("1", ["104"], ["1"], { from: david }),
            "No prize for this bracket"
          );
          await expectRevert(
            lottery.claimTickets("1", ["104"], ["0"], { from: david }),
            "No prize for this bracket"
          );
        });
        ...
        it("User cannot claim twice a winning ticket", async () => {
          await expectRevert(
            lottery.claimTickets("1", ["110"], ["5"], { from: david }),
            "Not the owner"
          );
        });
        //-----------------------------------------------------------------------------------------------------------------------------//
        it("Operator cannot start lottery if length is too short/long", async () => {
          ...
          let endTimeTarget = new BN(await time.latest())
            .add(_lengthLottery)
            .sub(new BN("10"));
          ...
          endTimeTarget = new BN(await time.latest())
            .add(_lengthLottery)
            .add(new BN("100"));
        ...
        it("Operator cannot start lottery if discount divisor is too low", async () => {
          ...
          _discountDivisor = new BN(await lottery.MIN_DISCOUNT_DIVISOR()).sub(
            new BN("1")
          );
        ...
        it("Operator cannot start lottery if treasury fee too high", async () => {
          ...
          _treasuryFee = new BN(await lottery.MAX_TREASURY_FEE()).add(
            new BN("1")
          );
        ...
        it("Operator cannot close lottery that is not started", async () => {
          await expectRevert(
            lottery.closeLottery("2", { from: operator }),
            "Lottery not open"
          );
        });
        ...
        it("Operator cannot draw numbers", async () => {
          await expectRevert(
            lottery.drawFinalNumberAndMakeLotteryClaimable("2", true, {
              from: operator,
            }),
            "Lottery not close"
          );
        ...
        it("User cannot buy tickets if one of the numbers is outside of range", async () => {
          ...
          await expectRevert(
            lottery.buyTickets(
              "2",
              ["1999999", "1929998", "1999999", "1999999", "1999998", "59999"],
              { from: bob }
            ),
            "Outside range"
          );
        ...
        it("User cannot claim tickets if same length for array arguments", async () => {
          await expectRevert(
            lottery.claimTickets("1", ["1999999", "1569999"], ["1"], {
              from: bob,
            }),
            "Not same length"
          );
        ...
        it("User cannot claim tickets if not over", async () => {
          await expectRevert(
            lottery.claimTickets("2", ["1999995", "1569995"], ["1", "1"], {
              from: bob,
            }),
        ...
        it("Cannot buy ticket when it is end time", async () => {
          ...
          await time.increaseTo(endTime);
          await expectRevert(
            lottery.buyTickets("2", ["1269956", "1269955"], { from: bob }),
            "Lottery is over"
          );
        });
        ...
        it("Cannot change generator number", async () => {
          await expectRevert(
            lottery.changeRandomGenerator(randomNumberGenerator.address, {
              from: alice,
            }),
        ...
        it("Operator cannot draw numbers if the lotteryId isn't updated in RandomGenerator", async () => {
          await randomNumberGenerator.setNextRandomResult("199999994", {
            from: alice,
          });
        ...
        it("Cannot claim for wrong lottery (too high)", async () => {
          await expectRevert(
            lottery.claimTickets("1", ["111"], ["5"], { from: david }),
            "TicketId too high"
          );
        });
        ...
        it("Cannot claim for wrong lottery (too low)", async () => {
          await expectRevert(
            lottery.claimTickets("2", ["110"], ["5"], { from: david }),
            "TicketId too low"
          );
        });
        ...
        it("Cannot claim for wrong lottery (too high)", async () => {
          await expectRevert(
            lottery.claimTickets("2", ["113"], ["5"], { from: david }),
            "TicketId too high"
          );
        });
        ...
        it("Lottery starts, close, and numbers get drawn without a participant", async () => {
          ...
          await expectRevert(
            lottery.claimTickets("3", ["113"], ["1"], { from: david }),
            "TicketId too high"
          );
        });

        it("Change the random generator (to existing one)", async () => {
          result = await lottery.changeRandomGenerator(
            randomNumberGenerator.address,
            { from: alice }
          );
          expectEvent(result, "NewRandomGenerator", {
            randomGenerator: randomNumberGenerator.address,
          });
        });

        it("Lottery starts with only 4 brackets with a prize, one user buys tickets", async () => {
          await randomNumberGenerator.setNextRandomResult("188888888", {
            from: alice,
          });
          ...
          await lottery.injectFunds("4", parseEther("1000"), {
            from: injector,
          });
          ...
          result = await lottery.buyTickets("4", _ticketsBought, {
            from: carol,
          });
        ...
        it("User claims first ticket", async () => {
          ...
          result = await lottery.claimTickets("4", ["113"], ["0"], {
            from: carol,
          });
        ...
        it("User cannot claim ticket in a bracket if equals to 0", async () => {
          await expectRevert(
            lottery.claimTickets("4", ["114"], ["1"], { from: carol }),
            "No prize for this bracket"
          );
          result = await lottery.claimTickets("4", ["114"], ["0"], {
            from: carol,
          });
        ...
        it("User claims 2 more tickets", async () => {
          result = await lottery.claimTickets("4", ["115", "118"], ["2", "5"], {
            from: carol,
          });
        ...
        it("User cannot claim ticket in a lower bracket if bracket above is not 0", async () => {
          await expectRevert(
            lottery.claimTickets("4", ["116"], ["2"], { from: carol }),
            "Bracket must be higher"
          );
          result = await lottery.claimTickets("4", ["116", "117"], ["3", "3"], {
            from: carol,
          });
      ...
      describe("Role exceptions", async () => {
        it("Owner can recover funds only if not CAKE token", async () => {
          ...
          result = await lottery.recoverWrongTokens(
            randomToken.address,
            parseEther("1"),
            { from: alice }
          );

          expectEvent(result, "AdminTokenRecovery", {
        ...
        it("Only operator can call operator functions", async () => {
          ...
          await expectRevert(
            lottery.closeLottery("2", { from: alice }),
            "Not operator"
          );
          await expectRevert(
            lottery.drawFinalNumberAndMakeLotteryClaimable("2", false, {
        ...
        it("Only owner/injector can call owner functions", async () => {
          ...
          await expectRevert(
            lottery.injectFunds("1", parseEther("10"), { from: operator }),
        ...
        it("Revert statements work in owner functions", async () => {
          await expectRevert(
            lottery.setMaxNumberTicketsPerBuy("0", { from: alice }),
            "Must be > 0"
          );
          await expectRevert(
            lottery.setOperatorAndTreasuryAndInjectorAddresses(
